...
root = Tk() root.geometry("500x555") root.title("UPG Utility") root.resizable(False,False)

def generator():
#this is the base code for the unique password generator full_dict = []
raw_pass = []


l1 = [chr(a) for a in range(97,123)] l2 = [chr(a) for a in range(65,91)] l3 = [chr(a) for a in range(48,58)] l4 = ["@","#","$","%"]
...
if(smallValue.get() == 1): raw_pass.append(random.choice(l1)) full_dict.extend(l1)

if(capValue.get() == 1): raw_pass.append(random.choice(l2)) full_dict.extend(l2)

if(numValue.get() == 1): raw_pass.append(random.choice(l3)) full_dict.extend(l3)


if(specialValue.get() == 1): raw_pass.append(random.choice(l4)) full_dict.extend(l4)

random.shuffle(full_dict)
 
x = int(passlengthValue.get()) shuffle_pass = []
final_pass = ""

start_val	=	smallValue.get()	+	capValue.get()	+	numValue.get()	+ specialValue.get()


for _ in range(start_val,x): raw_pass.append(random.choice(full_dict))

shuffle_pass = random.sample(raw_pass,len(raw_pass)) final_pass = final_pass.join(map(str,shuffle_pass)) print(final_pass)
password.delete('1.0', 'end') #clear the output text text widget password.insert(END, final_pass)
...
#Tkinter variables for storing entries passlengthValue = IntVar() smallValue = IntVar()
capValue = IntVar() numValue = IntVar() specialValue = IntVar()
...
password = Text(root, height = 3, width = 30, bg = "light grey") password.grid(row=1, column=1, padx=(0,25), sticky=E)
